...
const pathToFiles = __dirname.endsWith("backend") ? path.join(__dirname, 'files') : path.join(__dirname, 'backend', 'files')
...
// console.log("pathToMiraApp: " + pathToMiraApp)
...
const longVersion = '1.0.14 (November 24, 2024)'
...
// const useOpenAI = require('./useOpenAI');
// const useAnthropic = require('./useAnthropic');

//const useOpenAI = await import ('./useOpenAI.js')
//const useAnthropic = await import('./useAnthropic.js');
...
// app.use('/mira', express.static(path.join(__dirname, 'mira')));

// const staticDir = path.join(__dirname, 'dist')
// const staticDir = path.join(__dirname, 'public')
// app.use('/', express.static(staticDir))

// const miraDir = path.join(__dirname, 'mira')
// app.use('/mira', express.static(miraDir))


/*
function keyGenerator(request, response) {
	if (!request.ip) {
...
app.use(speedLimiter) // chat GPT global limit is OK. ??
// app.use(limiter)

// https://express-rate-limit.mintlify.app/guides/troubleshooting-proxy-issues
app.use(limiter) // ({keyGenerator})
