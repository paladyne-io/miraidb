    ...
    <q-card-section>
      ...
      <div class="row justify-end items-center q-mt-lg">
        ...
        />
          ...
          <span v-else>
            Not signed in. <router-link to="/login">Log in</router-link>
          </span>
        </div>
      </div>
    </q-card-section>
...
const { loading, error, downloadUserAvatar, getSignedInUserProfile } = useSupabaseAPI();
...
onMounted(async () => {
  ...
  if (userProfile.value) {
    if (userProfile.value.avatar_url) {
      userAvatar.value = await downloadUserAvatar(userProfile.value.avatar_url)
    }
    username.value = userProfile.value.username
  }
  console.log("newPost title: " + newPost.value.title)
});
